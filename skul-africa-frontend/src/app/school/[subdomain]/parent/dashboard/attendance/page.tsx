...
export default function ParentAttendancePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-white mb-2">Child's Attendance</h1>
        <p className="text-neutral-400">Monitor your child's attendance records</p>
      </div>
            ...
            {attendanceRecords.map((record, index) => (
              <div key={index} className="flex items-center justify-between p-4 border border-neutral-800 rounded-lg">
                <div className="flex items-center space-x-4">
                  <div className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusBg(record.status)} ${getStatusColor(record.status)}`}>
                    {record.status}
                  </div>
                  <div>
                    <p className="text-white font-medium">{record.subject}</p>
                    <p className="text-neutral-400 text-sm">{record.date} • {record.time}</p>
                  </div>
                </div>
              </div>
            ))}
